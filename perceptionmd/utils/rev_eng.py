...
def detect_shape(inputdata, dtype=np.float32, threeD=True):
    ...
    If the data is correlated, there is a good chance for proper detection.
    Shape of uncorrelated data is unrecoverable, the result will be gibberish.
...
def recognize_filetype(fn, def_type=np.float32, def_little_endian=True):
    """
...
def detect_filetype(filename, offset=0, types='all', endian='all', count=-1):
    ...
    for t in typelist:
        for e in endianlist:
            dtype = np.dtype(e + t)
            if mod(filesize, dtype.itemsize) != 0:
                continue
